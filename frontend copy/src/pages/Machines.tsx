...
import { gymService, type Machine as ApiMachine } from '@/services/gymService';
...
interface Machine extends ApiMachine {
  type: string;
}
...
export default function MachinesPage() {
  ...
  // WebSocket integration for real-time updates (only in development mode)
  ...
  useEffect(() => {
    ...
    const loadMachines = async () => {
      ...
      try {
        ...
        const machinesWithType = (response.machines || []).map(machine => ({
          ...machine,
          type: machine.name.toLowerCase().includes('treadmill') ? 'treadmill' :
                machine.name.toLowerCase().includes('bike') ? 'exercise-bike' :
                machine.name.toLowerCase().includes('elliptical') ? 'elliptical' :
                machine.category === 'cardio' ? 'treadmill' :
                machine.category === 'strength' ? 'weight-machine' : 'generic-machine'
        }));
        ...
        setError(`Failed to load machine data: ${error instanceof Error ? error.message : 'Unknown error'}`);
        setMachines([]);
      }
  ...
  // Update machines with real-time WebSocket data (development mode)
  useEffect(() => {
    // Only process WebSocket updates in development mode
  ...
  }, [webSocket.lastUpdate, appMode]);

  // Demo Mode: Automatic timestamp updates every 30 seconds (keep real availability data)
  useEffect(() => {
    if (appMode !== 'demo' || machines.length === 0) return;

    console.log('🎭 Demo mode: Setting up automatic timestamp updates (keeping real availability data)');

    const interval = setInterval(() => {
      setMachines(prevMachines => {
        if (prevMachines.length === 0) return prevMachines;

        // Randomly select 1-3 machines to update timestamps (realistic update frequency)
        ...
        for (let i = 0; i < numUpdates; i++) {
          ...
          do {
            randomIndex = Math.floor(Math.random() * prevMachines.length);
          } while (machineIndices.includes(randomIndex));

          machineIndices.push(randomIndex);
        }

        const updatedMachines = prevMachines.map((machine, index) => {
          if (machineIndices.includes(index)) {
            // Update timestamp to show as "just updated" (current time)
            const timestamp = Math.floor(Date.now() / 1000);

            console.log(`🎭 Demo timestamp update: ${machine.name} (${machine.machineId}) - showing as just updated`);
        ...
        console.log(`🎭 Demo mode: Updated timestamps for ${machineIndices.length} machine(s)`);
        return [...updatedMachines];
      });
    }, 30000); // 30 seconds

    // Initial update to set all machines to recent timestamps (1-10 minutes ago)
    const initialUpdate = setTimeout(() => {
      setMachines(prevMachines => {
        ...
        const updatedMachines = prevMachines.map(machine => {
          // Set each machine to a random recent timestamp (1-10 minutes ago)
          const minutesAgo = Math.floor(Math.random() * 10) + 1; // 1-10 minutes
          const recentTimestamp = now - (minutesAgo * 60);
        ...
        console.log('🎭 Demo mode: Set all machines to recent timestamps (1-10 minutes ago)');
        return [...updatedMachines];
      });
    }, 2000); // Faster initial update

    return () => {
      clearInterval(interval);
      clearTimeout(initialUpdate);
      console.log('🎭 Demo mode: Cleaning up timestamp updates');
    };
  }, [appMode, machines.length]);
