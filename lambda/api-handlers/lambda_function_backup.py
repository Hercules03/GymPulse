        ...
        if len(usage_data) > 0:
            try:
                ...
                if ml_forecast and 'forecast_hours' in ml_forecast:
                    current_hour_hk = hk_time.hour
                    next_hour = (current_hour_hk + 1) % 24

                    current_forecast = ml_forecast['forecast_hours'].get(str(current_hour_hk), {}).get('forecast', 50)
                ...
                current_hour_fallback = hk_time.hour
                current_usage = next((item['usage_percentage'] for item in usage_data if item['hour'] == current_hour_fallback), None)
                next_hour_usage = next((item['usage_percentage'] for item in usage_data if item['hour'] == (current_hour_fallback + 1) % 24), None)
