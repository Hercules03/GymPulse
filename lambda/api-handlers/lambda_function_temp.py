...
def handle_peak_hours_request(event, context, cors_headers):
    """
    Handle GET /branches/{branchId}/peak-hours - return peak hours forecast for a branch
    """
    try:
        ...
        print(f"🕐 Calculating peak hours forecast for branch: {branch_id}")
        ...
        # Calculate peak hours based on current data and patterns
        peak_hours = ""
        confidence = "medium"
        occupancy_forecast = {}

        # Dynamic peak detection based on real-time occupancy
        if current_occupancy_rate > 70:
            peak_hours = f"{current_hour}-{(current_hour + 2) % 24} (Peak Now)"
            confidence = "high"
        elif current_hour >= 6 and current_hour <= 9:
            peak_hours = "6-9 AM (Morning Rush)"
            confidence = "high" if current_occupancy_rate > 50 else "medium"
        elif current_hour >= 17 and current_hour <= 21:
            peak_hours = "5-9 PM (Evening Rush)"
            confidence = "high" if current_occupancy_rate > 60 else "medium"
        elif current_hour >= 12 and current_hour <= 14:
            peak_hours = "12-2 PM (Lunch Rush)"
            confidence = "medium" if current_occupancy_rate > 40 else "low"
        else:
            # Don't set to "Off-Peak Hours" - will be handled by forecast logic below
            peak_hours = ""
            confidence = "medium"

        # Generate 6-hour occupancy forecast
        for i in range(6):
            forecast_hour = (current_hour + i) % 24

            # Base forecast on current occupancy
            forecast_occupancy = current_occupancy_rate

            # Apply time-based multipliers
            if forecast_hour >= 6 and forecast_hour <= 9:  # Morning peak
                forecast_occupancy *= 1.3
            elif forecast_hour >= 17 and forecast_hour <= 21:  # Evening peak
                forecast_occupancy *= 1.5
            elif forecast_hour >= 12 and forecast_hour <= 14:  # Lunch peak
                forecast_occupancy *= 1.2
            elif forecast_hour >= 22 or forecast_hour <= 5:  # Night/early morning
                forecast_occupancy *= 0.3
            else:  # Off-peak
                forecast_occupancy *= 0.8

            # Apply some randomness for realism (±15%)
            import random
            variance = random.uniform(-0.15, 0.15)
            forecast_occupancy *= (1 + variance)
        ...
        # Determine next peak time for enhanced forecasting
        next_peak_hour = None
        next_peak_name = ""
        if current_hour < 6:
            next_peak_hour = 6  # Morning rush
            next_peak_name = "Morning Rush (6-9 AM)"
        elif current_hour < 12:
            next_peak_hour = 12  # Lunch rush
            next_peak_name = "Lunch Rush (12-2 PM)"
        elif current_hour < 17:
            next_peak_hour = 17  # Evening rush
            next_peak_name = "Evening Rush (5-9 PM)"
        else:
            next_peak_hour = 6 + 24  # Next morning (tomorrow)
            next_peak_name = "Morning Rush (6-9 AM Tomorrow)"

        hours_until_peak = (next_peak_hour - current_hour) % 24

        # Create user-friendly forecast message based on current vs future peak
        is_current_peak = bool(peak_hours)  # If peak_hours is set, we're currently in peak

        if is_current_peak:
            # Currently in peak hours - show current peak
            forecast_message = peak_hours
        else:
            # Not in peak hours - show next upcoming peak
            if hours_until_peak == 0:
                forecast_message = next_peak_name
            elif hours_until_peak == 1:
                forecast_message = f"Next: {next_peak_name} (in 1 hour)"
            elif hours_until_peak <= 6:
                forecast_message = f"Next: {next_peak_name} (in {hours_until_peak}h)"
            else:
                forecast_message = f"Next: {next_peak_name}"

        peak_forecast = {
            'branchId': branch_id,
            'currentHour': current_hour,
            'currentOccupancy': round(current_occupancy_rate, 1),
            'peakHours': forecast_message,  # Use forecast message instead of current peak
            'confidence': confidence,
            'occupancyForecast': occupancy_forecast,
            'nextPeakIn': hours_until_peak,
            'nextPeakName': next_peak_name,
            'totalMachines': total_machines,
            'generatedAt': hk_time.isoformat()
        }

        print(f"✅ Peak hours forecast generated: {peak_hours} (confidence: {confidence})")
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json', **cors_headers},
            'body': json.dumps({'error': 'Failed to calculate peak hours', 'details': str(e)})
        }
